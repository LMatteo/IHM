...
public class AdminNewsController {
    ...
    public void placeNews() throws MalformedURLException {
        for (News n : listNews.getNewsList()) {
            if (newsCorrespondance.containsKey(n.getPosition())) {
                newsCorrespondance.get(n.getPosition()).setImage(new Image(new File("data/centre/images/" + n.getFrench() + ".png").toURI().toURL().toString()));
            }
        }
        for (ImageView iv : newsCorrespondance.values()) {
            if (iv.getImage() == null) {
                iv.setImage(new Image(new File("data/centre/images/add.png").toURI().toURL().toString()));
            }
        }
    }
    ...
    void modifyPanel(MouseEvent event) throws IOException {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/centre/newSelector.fxml"));
